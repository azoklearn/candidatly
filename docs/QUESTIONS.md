...
| C76 | Confirmation de l'email à l'inscription | Retirée le 11 septembre 2026 à la demande de l'owner, car les emails de confirmation n'arrivaient pas. Le code connecte déjà directement l'étudiant quand Supabase renvoie une session ; le réglage « Confirm email » se coupe dans le tableau de bord Supabase (Authentication > Sign In / Providers > Email), l'offre gratuite refusant les changements de configuration d'email par la CLI sans serveur d'envoi. Sans serveur d'envoi personnalisé, la messagerie intégrée de Supabase n'écrit qu'aux membres de l'équipe du projet, avec quelques envois par heure. Réglage coupé par l'owner le 11 septembre 2026 : l'inscription connecte directement l'étudiant | N'importe qui peut s'inscrire avec une adresse qui n'est pas la sienne ; à réactiver avec un serveur d'envoi (Resend, Brevo…) avant l'ouverture publique (`docs/RUNBOOK.md`) |
| C77 | Nouvelle page d'accueil | Maquette statique de l'owner (HTML, CSS, JS) intégrée le 11 septembre 2026 dans `app/(landing)` : même design, animations, palette blanc cassé et bleu, CTA brillant, polices DM Sans, DM Mono et Instrument Serif servies par `next/font`, CSS isolé sous `.lp`. Contenus rendus exacts, le site étant public : note « 4,8/5 par +1 800 étudiants », avis signés et « 73 % » remplacés par la preuve sociale calculée (C73) et par « Nos engagements » ; Qonto et Back Market remplacés par des entreprises fictives signalées ; alertes, envoi « chaque matin », lecture des sites d'entreprises et salaires retirés car ces fonctions n'existent pas ; liste d'attente remplacée par le questionnaire (`/signup`) ; liens Instagram, LinkedIn et TikTok vides remplacés par les pages légales ; lien « Se connecter » ajouté. Le tutoiement de la maquette est conservé sur l'accueil, l'application reste au vouvoiement | Design fidèle, aucune allégation invérifiable ; à harmoniser si l'owner veut un seul registre |
| C78 | Confirmation de l'email réactivée | L'owner réactive le réglage « Confirm email » le 11 septembre 2026, après C76. Le lien de l'email ramène sur `/auth/callback`, qui connecte l'étudiant et l'envoie au questionnaire ; ouvert sur un autre appareil, le lien confirme quand même l'adresse et la page de connexion invite à se connecter avec son mot de passe. Sans serveur d'envoi personnalisé, seuls les membres de l'équipe Supabase reçoivent l'email, à quelques envois par heure | À compléter par un serveur d'envoi (Resend, Brevo…) avant d'accueillir de vrais étudiants (`docs/RUNBOOK.md`) |
| C79 | Vérification de l'email supprimée définitivement | Décision de l'owner du 11 septembre 2026, après C76 et C78. La messagerie intégrée de Supabase refuse d'écrire hors de l'équipe du projet et se limite à 2 emails par heure (documentation Supabase, « Send emails with custom SMTP »). L'inscription ne dépend plus du réglage « Confirm email » : si Supabase demande encore une confirmation, le serveur confirme le nouveau compte avec la clé secrète et connecte l'étudiant ; une adresse déjà inscrite n'est jamais touchée | Aucune adresse n'est vérifiée ; pas de récupération de mot de passe par email sans serveur d'envoi ; test de bout en bout de l'inscription ajouté |
| C80 | Entreprises à contacter quand les offres manquent | Demande de l'owner du 11 septembre 2026 : quand la recherche trouve peu d'offres, proposer des entreprises susceptibles de recruter, sans jamais écrire « aucune offre trouvée ». Source : le tableau `recruiters` de la recherche API Alternance, entreprises repérées par La bonne alternance d'après leurs recrutements d'alternants (150 au plus par recherche ; 150 sur 150 bien formées pour un développeur à Paris le 11 septembre 2026). Elles sont stockées par clé de recherche dans `hiring_companies`, remplacées à chaque synchronisation et purgées après 3 jours sans recherche. En dessous de 10 offres, la page des offres les affiche dans une section « Entreprises à fort potentiel d'embauche », classées par distance, avec un lien de candidature spontanée vers La bonne alternance | Jamais présentées comme des offres : pas de score, pas d'intitulé de poste, mention « Candidature spontanée » visible, pour que l'étudiant ne cherche pas une annonce qui n'existe pas. La page ne dit jamais qu'aucune offre n'a été trouvée |
| C81 | Plus de mention de bêta ni de gratuité | Demande de l'owner du 11 septembre 2026, avec le nouveau domaine https://candidatly.app. Retirés de l'accueil : « Rejoindre la bêta » (remplacé par « Trouver mon stage/alternance »), la carte « 0 € pendant toute la bêta » (remplacée par un fait vérifiable : chaque changement de la lettre est montré), la pastille « Bêta ouverte pour septembre » et la note « Accès gratuit pendant la bêta » ; phrase de repli de la pastille de preuve, page Crédits et conditions d'utilisation alignées | Les conditions ne mentionnent plus la gratuité mais gardent l'engagement d'annoncer toute formule payante à l'avance, avec l'accord de l'utilisateur. Un test de bout en bout vérifie l'absence de « bêta », « 0 € » et « gratuit » sur l'accueil |
| C82 | Tarifs et choix du forfait après le questionnaire | Demande de l'owner du 11 septembre 2026. Trois forfaits : Basic (offres de la zone et du domaine, 3 recherches par jour), Plus (lettre adaptée à chaque entreprise, candidature prête et suivi, 10 recherches par jour), Premium (recherches illimitées). Prix d'origine 14,99 / 24,99 / 39,99 € par mois, baissés le 12 septembre 2026 à la demande de l'owner à 9,99 / 14,99 / 19,99 € par mois (99,90 / 149,90 / 199,90 € par an) ; les plans Whop doivent être corrigés en même temps, sinon le paiement refuse de démarrer. Annuel = 10 mois (2 mois offerts, choix de l'owner) ; prix par jour affiché (mois de 30 jours, année de 365). Page publique `/tarifs` ; après le questionnaire, `/forfait` annonce le nombre d'offres trouvées, et d'entreprises qui recrutent, puis propose les forfaits. Pas de paiement : le choix est enregistré (`profiles.chosen_plan`, écrit avec la clé secrète) et l'accès reste ouvert jusqu'au branchement de Stripe (A5), choix de l'owner ; les limites par jour ne sont pas encore appliquées | Prix barré : uniquement le prix mensuel du même forfait, à côté de l'équivalent mensuel de la formule annuelle, car un prix de référence jamais pratiqué est une pratique commerciale trompeuse. « Le plus choisi » n'apparaît qu'à partir de 20 choix, sur le forfait réellement en tête ; avant, le forfait Plus porte « Recommandé ». CV adapté à chaque entreprise, envoi direct et optimisation du CV sont marqués « Bientôt ». Faire payer l'accès aux offres de l'API Alternance est ce que ses conditions interdisent (A1, risque accepté par l'owner) |
| C83 | Paiement par Whop | Décision de l'owner du 11 septembre 2026 (« on utilise whop pour le paiement »), à la place de Stripe prévu par le brief. Paiement obligatoire, choix de l'owner : sans abonnement actif, les offres et les candidatures renvoient vers `/forfait` ; le compte, l'export et la suppression restent accessibles. Chaque forfait ouvre ses fonctions : lettre adaptée pour Plus et Premium, 3 recherches par jour pour Basic, 10 pour Plus. Choisir un forfait crée une session de paiement Whop (API v1, `checkout_configurations`) portant l'identifiant de l'étudiant en métadonnée ; le webhook signé (Standard Webhooks) relit l'abonnement dans l'API et l'enregistre dans `subscriptions`. Les tables Stripe, jamais utilisées, sont renommées (`billing_events`) ; `billing_plans` relie nos forfaits aux plans Whop créés par `npm run whop:setup` | Le mur de paiement ne s'active qu'avec `WHOP_API_KEY` et `WHOP_WEBHOOK_SECRET` : avant, l'accès reste ouvert. Aucune bibliothèque ajoutée (appels HTTP et signature dans `lib/billing`). Comptes exemptés par `profiles.billing_exempt`. Frais et TVA côté Whop à vérifier par l'owner ; faire payer l'accès aux offres de l'API Alternance reste le point A1 |
| C84 | Mesure d'audience | Demande de l'owner du 11 septembre 2026 (« ajoute vercel analytics ») : bibliothèque `@vercel/analytics` 2.0, validée par cette demande, composant chargé dans `app/layout.tsx`. Vercel Web Analytics compte les pages vues sans cookie. Avant tout envoi, l'adresse de la page perd ses paramètres et son fragment (jetons de connexion, filtres saisis) et ses identifiants deviennent `[id]` (`lib/analytics.ts`) | Aucune donnée n'est collectée tant que l'owner n'a pas activé Analytics dans le projet Vercel. Politique de confidentialité mise à jour (mesure d'audience, Whop comme sous-traitant du paiement) |
| C85 | Événements personnalisés | Demande de l'owner du 11 septembre 2026 (« ajoute les custom event analytics »). Côté serveur (`lib/analytics-server.ts`, uniquement sur Vercel) : Inscription, Questionnaire terminé, Forfait choisi et Paiement démarré (forfait, facturation), Abonnement activé et Abonnement terminé (depuis le webhook Whop), Offres actualisées, Candidature préparée, Candidature envoyée. Côté navigateur (`components/tracked-link.tsx`) : Clic inscription (emplacement du bouton sur l'accueil), Candidature spontanée, Site de l'offre ouvert. Noms en français pour le tableau de bord de l'owner, liste dans `lib/analytics.ts` | Vercel ne garde les événements personnalisés que sur l'offre Pro, avec 2 propriétés au plus par événement : aucun événement n'en porte davantage. Aucune donnée personnelle dans les propriétés ; un échec d'envoi est journalisé sans bloquer l'action |
| C86 | Ton de l'accueil et mention « IA » | Demande de l'owner du 12 septembre 2026 : textes plus agressifs et accroche « trouve ton alternance grâce à l'IA ». Les textes sont réécrits (accroche « Arrête de postuler dans le vide. Décroche. », étapes et section finale plus directes, boutons « Décrocher mon alternance » et « Prendre une longueur d'avance »). La mention « IA » n'est pas écrite : aucun modèle ne tourne aujourd'hui (pas de clé Anthropic, lettre `rules-v1`, métiers par le classement ROME), et l'écrire sur une page payante serait une allégation trompeuse | À faire pour pouvoir l'écrire : clé `ANTHROPIC_API_KEY` fournie par l'owner, puis adaptation de la lettre et choix des métiers par le modèle ; l'accroche sera changée dans la foulée. Choix de l'owner : textes agressifs d'abord, IA juste après |
| C87 | Contacter le recruteur | Demande de l'owner du 12 septembre 2026. Aucune source ne publie d'adresse email de recruteur (mesure du 10 septembre 2026, `docs/API_ALTERNANCE.md` section 0) : les canaux réels sont le téléphone quand l'offre en donne un (14 offres sur 70 le 12 septembre), le site de l'offre et le site de l'entreprise. La fiche d'une offre affiche une carte « Contacter le recruteur » avec le numéro cliquable (`tel:`), en précisant s'il s'agit de l'employeur ou du centre de formation quand l'offre est déléguée ; les entreprises à fort potentiel d'embauche gagnent une colonne `phone` et un bouton « Appeler ». Numéros normalisés par `lib/offers/contact.ts` | Rien n'est inventé : sans téléphone ni site, la carte dit que le site de l'offre est le seul canal. L'envoi de candidature par l'API Alternance reste fermé faute d'habilitation (A1), donc aucun bouton ne promet un envoi direct |
| C88 | Aperçu flouté avant paiement | Demande de l'owner du 12 septembre 2026 : après le questionnaire, annoncer le nombre d'offres et d'entreprises trouvées et montrer un aperçu flouté. Le flou CSS s'enlevant depuis les outils du navigateur, l'aperçu ne contient pas les données à cacher : le serveur n'envoie que la ville, le type de contrat, la fraîcheur et le score (`lib/offers/teaser.ts`), et les barres floutées sont vides (`.teaser-bar`). L'intitulé du poste et l'employeur ne quittent jamais le serveur tant que le forfait n'est pas actif | Un test de bout en bout vérifie que le titre de l'offre est absent du HTML de `/forfait`. L'aperçu ne s'affiche pas aux comptes qui ont déjà un abonnement, qui voient les offres complètes |
| C89 | CV et lettre facultatifs | Demande de l'owner du 13 septembre 2026 : pouvoir passer l'étape CV et lettre du questionnaire. Bouton « Passer cette étape » ; le choix est mémorisé dans `profiles.documents_skipped_at` (modifiable par l'étudiant), et l'étape compte comme faite. Les documents s'ajoutent plus tard depuis la page Compte | Sans CV, le calcul des correspondances ignore simplement les mots-clés du CV. Sans lettre de base, la lettre adaptée ne peut pas exister : la fiche d'offre affiche « Ajouter ma lettre » vers le compte au lieu de « Préparer ma candidature », qui renvoyait auparavant l'étudiant sans explication |
